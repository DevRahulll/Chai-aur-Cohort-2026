<a href="https://devrahulll.hashnode.dev/">
  <h1 align="center">Blogs Archive 📔</h1>
</a>

<p align="center">
  Showcase of my journey to building a solid Foundation.
</p>

<br>

### 1. [Why Version Control Exists](https://devrahulll.hashnode.dev/git1)

  <!-- <a href="https://devrahulll.hashnode.dev/git1">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/b1-banner.png">
    <p>
  </a> -->
<br>

### 2. [Git for Beginners](https://devrahulll.hashnode.dev/git2)

  <a href="https://devrahulll.hashnode.dev/git2">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/gitforbeginnersbanner.webp">
    <p>
  </a>

<br>

### 3. [Inside Git: How it works](https://devrahulll.hashnode.dev/git3)

  <a href="https://devrahulll.hashnode.dev/git3">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/insideGitBanner.webp">
    <p>
  </a>

<br>

### 4. [Understanding Network Devices](https://devrahulll.hashnode.dev/understanding-network-devices)

  <a href="https://devrahulll.hashnode.dev/understanding-network-devices">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/networkDevices.webp">
    <p>
  </a>

<br>

### 5. [How DNS Resolution Works](https://devrahulll.hashnode.dev/how-dns-resolution-works)

  <a href="https://devrahulll.hashnode.dev/how-dns-resolution-works">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/dnsResolutionbanner.webp">
    <p>
  </a>

<br>

### 6. [DNS Record Types Explained](https://devrahulll.hashnode.dev/dns-record-types)

  <a href="https://devrahulll.hashnode.dev/dns-record-types">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/dnsRecordTypesBanner.webp">
    <p>
  </a>

<br>

### 7. [Getting Started with cURL](https://devrahulll.hashnode.dev/curl)

  <a href="https://devrahulll.hashnode.dev/curl">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/curlBanner.webp">
    <p>
  </a>

<br>

### 8. [TCP Working: 3-way Handshake](https://devrahulll.hashnode.dev/tcp-working-3-way-handshake)

  <a href="https://devrahulll.hashnode.dev/tcp-working-3-way-handshake">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/tcpbanner.png">
    <p>
  </a>

<br>

### 9. [TCP vs UDP](https://devrahulll.hashnode.dev/tcp-vs-udp)

  <a href="https://devrahulll.hashnode.dev/tcp-vs-udp">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/tcpvsudp.png">
    <p>
  </a>

<br>

### 10. [How a Browser Works](https://devrahulll.hashnode.dev/how-browsers-works)

  <a href="https://devrahulll.hashnode.dev/how-browsers-works">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/browserBanner.png">
    <p>
  </a>

<br>

### 11. [Understanding HTML Tags and Elements](https://devrahulll.hashnode.dev/html-tags-and-elements)

  <a href="https://devrahulll.hashnode.dev/html-tags-and-elements">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/hmtlTagsBanner.webp>
    <p>
  </a>

<br>

### 12. [Emmet for HTML](https://devrahulll.hashnode.dev/emmet)

  <a href="https://devrahulll.hashnode.dev/emmet">
      <p align=center>
      <img width = "500px" alt="banner" src="./assets/emmetbanner.png">
    <p>
  </a>

<br>

### 13. [CSS Selectors](https://devrahulll.hashnode.dev/css-selectors)

  <a href="https://devrahulll.hashnode.dev/css-selectors">
      <p align=center>
      <img width = "500px" alt="Jio Network blocking the view? Network switch reveals the magic!" src="./assets/cssSelectorsBanner.webp">
    <p>
  </a>

<br>
