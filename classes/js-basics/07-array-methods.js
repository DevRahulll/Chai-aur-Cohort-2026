...
// foreach does not return anything but map return a new array
...
const ticketNumbers = [100, 25, 3, 43, 9];
...
const mildReport = kitchenOrders.filter((order) => !order.spicy).map((order) => ({
    dish: order.dish,
    total: order.price * order.qty
}))
    .toSorted();
console.log(...mildReport)
const mildarray = Object.entries(mildReport);
console.log(mildarray)
